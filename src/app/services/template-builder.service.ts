...
<body>
  ...
  private generateHeaderSection(config: EmailTemplateConfig): string {
    ...
    return `
        <div class="header-section">
            <img src="${config.headerImage.url}" alt="${config.headerImage.alt || 'Header'}" class="header-image">
        </div>
    `;
  }
  ...
  private generateButtonSection(config: EmailTemplateConfig): string {
    ...
    return `
        <div class="content-section" style="text-align: center;">
            <a href="${buttonLink}" target="_blank" class="cta-button" style="cursor: pointer;">${config.button.text}</a>
        </div>
    `;
  }
    ...
    return `
        <div class="content-section">
            <div class="gps-section">
                <div class="gps-title">${config.gpsCoordinates.label || 'COORDONNÉES GPS'}</div>
                <div class="gps-subtitle">ENTRÉE PRINCIPALE</div>
  ...
  private generateSocialSection(config: EmailTemplateConfig, socialLinks: any[]): string {
    ...
    const links = socialLinks.map(link => {
      ...
      return `<a href="${link.url}" target="_blank" title="${link.platform}">${icon}</a>`;
    }).join('');
  ...
  private generateFooterSection(): string {
    return `
        <div class="footer">
            <p>Pour être retiré de notre liste de diffusion, vous pouvez vous <a href="{{UNSUBSCRIBE_LINK}}" style="color: #1976D2; text-decoration: underline;">désinscrire</a></p>
        </div>
    `;
  }
